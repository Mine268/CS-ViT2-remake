...
17. TI 的科研验证需要独立消融：至少比较 `MODEL.ti.enabled=false/true`，并在不同模型规模（DINOv2-L、DINOv3-L/16、可选 DINOv3-H+/16）和不同训练源（full stage1、ego-only、AssemblyHands-only/HOT3D-only）下重复。评价应覆盖 aggregate AssemblyHands val 指标和 challenge-stratified 指标，避免只凭单个 DINOv3-L run 判断 TI 有效。
18. 旧的 CS-ViT2 在 `/data_1/renkaiwen/CS-ViT2` 下。
