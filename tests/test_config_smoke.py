...
from src.train.engine import (
    create_accelerator,
    get_linear_warmup_constant_schedule,
    resolve_tracker_step,
    setup_model,
)
...
def test_stage1_config_parses():
    ...
    assert cfg.MODEL.ti.enabled is False
    assert cfg.MODEL.ti.apply_stage1 is True
    assert cfg.MODEL.ti.apply_stage2 is False
...
def test_dinov3_large_configs_parse_with_expected_overrides():
    ...
    assert stage1.TRAIN.sample_per_device == 42
    assert stage1.TRAIN.backbone_lr == 1e-5
    assert stage2.MODEL.stage == "stage2"
...
def test_tracker_step_uses_seen_samples_and_clamps_final_batch():
    """SwanLab progress should use consumed sample count rather than raw optimizer steps."""
    assert resolve_tracker_step(global_step=10, total_batch=126, total_samples=25_600_000) == 1_260
    assert resolve_tracker_step(global_step=4, total_batch=32, total_samples=100) == 100
    assert resolve_tracker_step(global_step=10, total_batch=126, total_samples=0) == 10
...
def test_stage1_ti_config_instantiates_ti_module():
    """Stage1 should wire the TI module cleanly when the feature branch is enabled."""
    config_dir = str((Path(__file__).resolve().parents[1] / "config").resolve())
    with initialize_config_dir(version_base=None, config_dir=config_dir):
        cfg = compose(config_name="stage1", overrides=["MODEL.ti.enabled=true"])

    net = setup_model(cfg)
    assert net.ti_transform is not None
    assert net.ti_apply_stage1 is True
    assert net.ti_apply_stage2 is False
